...
const NODE_JS_FILE = `${ DIR_MAIN_FILES }${ FILE_NAME_NO_EXT }${ TAIL_TEXT }`; // Current-Active-Working-File
...
fs.copyFileSync(`${ NODE_JS_FILE }`, `${ DIR_MAIN_FILES }New_${ FILE_NAME_NO_EXT }${ TAIL_TEXT }`);

// Append Min & Max values to Node-file. THIS IS THE ONLY WRITING TO THE NODE FILE IN APP

import(`./main_files/${ FILE_NAME_NO_EXT }${ TAIL_TEXT }`).then((pMemZero) =>
{
  fs.openSync(`${ DIR_MAIN_FILES }New_${ FILE_NAME_NO_EXT }${ TAIL_TEXT }`, "r+");
  ...
  for (const minMaxAddress in MIN_MAX_ADRS)
  {
    ...
    {
      fs.appendFileSync(`${ DIR_MAIN_FILES }New_${ FILE_NAME_NO_EXT }${ TAIL_TEXT }`,
...
const newFile = fs.promises.openSync(`${ DIR_MAIN_FILES }New_${ FILE_NAME_NO_EXT }${ TAIL_TEXT }`, "r+");
const newFileData = fs.readFileSync(newFile, {encoding: "utf8"})
newFileData
  .toString().split(";").forEach((exportVar) =>
  {
    const allVariablesREGEX = /exports\.(\w+) = "(.*)"/gmi; // ALL VAR REGEX
    let tempRegExVar;
...
if (newFileData.includes("exports.P2_Combo_Meter_Value_Max ="))
// if (newFileData.includes("exports.Total_Frames ="))
{
...
{
  const readFileForChecking = fs.readFileSync(`${ DIR_MAIN_FILES }New_${ FILE_NAME_NO_EXT }${TAIL_TEXT}`, {encoding: "utf8"}); // Expected to exectute after synchronous pause & write
  if (readFileForChecking.match("P2_C_Y_Velocity_Max") && readFileForChecking.match("P1_A_X_Gravity_Min"))
  {
    const playerDataAll = []
    const getFile = fs.readFileSync(`${ DIR_MAIN_FILES }New_${ FILE_NAME_NO_EXT }${TAIL_TEXT}`, "utf8",);
  ...
  function writeNewStates()
  {
    ...
    for (tempPlayerValue = 1; tempPlayerValue < 3; tempPlayerValue++)
    {
      ...
      for (let playerSlotI = 0; playerSlotI < 3; playerSlotI++)
      {
        ...
        var ROM_CHOICEE = Object.values(allDataObject.State_ROM_09_ChoiceE);
        for (const arrayWithROMData in ROM_CHOICEE) // 3 arrays
        {
          for (var clipLen = 0; clipLen < CLIP_LENGTH; clipLen++)
          {
            if ((getAirborne)[arrayWithROMData][clipLen] == 0)
          ...
          for (var clipLen = 0; clipLen < CLIP_LENGTH; clipLen++)
          {
            if (ROM_CHOICEE[arrayWithROMData][clipLen] == 1)
            {
              ...
              {
                ROM_CHOICEE[arrayWithROMData][clipLen] = "\'MK\'";
              }
              ...
              {
                ROM_CHOICEE[arrayWithROMData][clipLen] = "\'DLK\'";
              }
            }
          }
          // Count the frames of LKs using tempCounter
          for (var clipLen = 0; clipLen < CLIP_LENGTH; clipLen++)
          {
            if (ROM_CHOICEE[arrayWithROMData][clipLen] == '\'DLK\'')
            {
              tempCounter += 1;
            }
            // Stop on encountering a MK
            else if (ROM_CHOICEE[arrayWithROMData][clipLen] == '\'MK\'') // We hit a MK
            {
              // Lookahead
              ...
              for (let negativeI = 1; negativeI < tempCounter + 1; negativeI++)
              {
                if (ROM_CHOICEE[arrayWithROMData][clipLen - negativeI] == '\'DLK\'')
                {
          ...
          {
            if ((ROM_CHOICEE[arrayWithROMData][clipLen] != 65535)
              && (ROM_CHOICEE[arrayWithROMData][clipLen] >= 13))
            {
