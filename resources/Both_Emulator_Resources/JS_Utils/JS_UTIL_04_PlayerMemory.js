...
export default function writePlayerMemory(PlayerOneOrPlayerTwo, pMemAdr, write) {
  if (enableLogging) {
    console.log(pMemAdr);
  }
  for (const tempObj in updatedObj) {
    ...
    for (let clipLen = 0; clipLen < CLIP_LENGTH; clipLen++) {
      ...
      if ((parseFloat(Object.values(p1OrP2Obj)[0][clipLen]) === 0)
        ...
        finalValArr[0].push(
          updatedObj[tempObj][`${Object.keys(p1OrP2Obj)[0]}${pMemAdr}`]
            .split(',')[clipLen],
        );
        finalValArr[1].push(
          updatedObj[tempObj][`${Object.keys(p1OrP2Obj)[1]}${pMemAdr}`]
            .split(',')[clipLen],
        );
        finalValArr[2].push(
          updatedObj[tempObj][`${Object.keys(p1OrP2Obj)[2]}${pMemAdr}`]
            .split(',')[clipLen],
        );
        ...
        finalValArr[0].push(
          updatedObj[tempObj][`${Object.keys(p1OrP2Obj)[0]}${pMemAdr}`].split(',')[clipLen],
        );
        finalValArr[1].push(
          updatedObj[tempObj][`${Object.keys(p1OrP2Obj)[1]}${pMemAdr}`].split(',')[clipLen],
        );
      } else if ((parseFloat(Object.values(p1OrP2Obj)[0][clipLen]) === 0)
        && (parseFloat(Object.values(p1OrP2Obj)[1][clipLen]) !== 0)
        && (parseFloat(Object.values(p1OrP2Obj)[2][clipLen]) === 0)
        ...
        finalValArr[1].push(
          updatedObj[tempObj][`${Object.keys(p1OrP2Obj)[2]}${pMemAdr}`].split(',')[clipLen],
        );
      } else if ((parseFloat(Object.values(p1OrP2Obj)[0][clipLen]) !== 0)
        && (parseFloat(Object.values(p1OrP2Obj)[1][clipLen]) === 0)
        && (parseFloat(Object.values(p1OrP2Obj)[2][clipLen]) === 0)
...
// get the first address inside of giant object in the first object
// playerMemoryEntries.forEach((pMemAdr) => {
//   writePlayerMemory(1, pMemAdr.toString(), 1);
//   writePlayerMemory(2, pMemAdr.toString(), 1);
// });
