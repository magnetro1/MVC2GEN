 ...
 */
export default async function writeNewStates(giantObjectCopy) {
  for (const tempObj in giantObjectCopy) {
    const DIR_OUTPATH = `${DIR_EXPORT_TO_AE}${tempObj}/`;
    const CLIP_LENGTH = giantObjectCopy[tempObj].Total_Frames.split(',').length;
      ...
      for (let abcIDX = 0; abcIDX < 3; abcIDX++) {
        ...
        for (let state = 0; state < Object.entries(newStatesObj).length; state++) {
          fs.promises.writeFile(
            `${DIR_OUTPATH}${p1OrP2}_${Object.keys(newStatesObj)[state]}.js`,
