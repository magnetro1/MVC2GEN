 ...
 * @param {string|null} objName string or null. If writing, the name will come from the array.
 * @param {number|string} P1OrP2 number or string, ex: 1 or "P1" or "1"
 * @param {string|object} pMem string, ex: "Health_Big", "X_Velocity" or object of arrays
...
export default function getPMem(updObj, objName, P1OrP2, pMem, write) {
  // console.log(objectName);
  if (write === 0) {
    if (enableLogging) {
      console.log(objName, P1OrP2, pMem, write);
    }
    ...
    */
    ...
    for (let clpLn = 0; clpLn < CLIP_LENGTH; clpLn++) {
      ...
      if ((parseFloat(Object.values(p1P2Obj)[0][clpLn]) === 0)
        ...
        resArr[0].push(
          updObj[objName][`${Object.keys(p1P2Obj)[0]}${pMem}`]
            .split(',')[clpLn],
        );
        resArr[1].push(
          updObj[objName][`${Object.keys(p1P2Obj)[1]}${pMem}`]
            .split(',')[clpLn],
        );
        resArr[2].push(
          updObj[objName][`${Object.keys(p1P2Obj)[2]}${pMem}`]
            .split(',')[clpLn],
        );
        ...
        resArr[0].push(
          updObj[objName][`${Object.keys(p1P2Obj)[0]}${pMem}`].split(',')[clpLn],
        );
        resArr[1].push(
          updObj[objName][`${Object.keys(p1P2Obj)[1]}${pMem}`].split(',')[clpLn],
        );
      } else if ((parseFloat(Object.values(p1P2Obj)[0][clpLn]) === 0)
        && (parseFloat(Object.values(p1P2Obj)[1][clpLn]) !== 0)
        && (parseFloat(Object.values(p1P2Obj)[2][clpLn]) === 0)
        ...
        resArr[1].push(
          updObj[objName][`${Object.keys(p1P2Obj)[2]}${pMem}`].split(',')[clpLn],
        );
      } else if ((parseFloat(Object.values(p1P2Obj)[0][clpLn]) !== 0)
        && (parseFloat(Object.values(p1P2Obj)[1][clpLn]) === 0)
        && (parseFloat(Object.values(p1P2Obj)[2][clpLn]) === 0)
