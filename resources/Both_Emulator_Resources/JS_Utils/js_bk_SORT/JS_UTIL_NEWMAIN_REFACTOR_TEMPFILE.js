...
const knownName = 'Magneto_Movelist';
      ...
      .then((pMem) => {
        ...
        */
        ...
        function writeP1P2Addresses() {
          ...
          for (const miscAdrI in P1P2_ADDRESSES) {
            pMem[P1P2_ADDRESSES[miscAdrI]].split(',').forEach((clipLenEntry) => // accessing pMem object key by string, splitting its content, and pushing each frame's value
            {
              miscAdrArray[0].push(clipLenEntry);
            });

            if (!fs.existsSync(`${DIR_OUTPATH}${P1P2_ADDRESSES[miscAdrI]}.js`)) {
              fs.writeFileSync(`${DIR_OUTPATH}${P1P2_ADDRESSES[miscAdrI]}.js`,
        ...
        **/
        ...
        function writeP1P2Addresses() {
          ...
          for (const miscAdrIterator in P1P2_ADDRESSES) {
            pMem[P1P2_ADDRESSES[miscAdrIterator]].split(',').forEach((address) => // accessing pMem object key by string, splitting its content, and pushing each array element
            {
              miscAdrArray[0].push(address);
            });

            if (!fs.existsSync(`${DIR_OUTPATH}${P1P2_ADDRESSES[miscAdrIterator]}.js`)) {
              fs.writeFileSync(`${DIR_OUTPATH}${P1P2_ADDRESSES[miscAdrIterator]}.js`,
                `var result = [];\nresult[0] = [${miscAdrArray}];`,
                { encoding: 'utf8' });
              miscAdrArray[0] = []; // clear the array for the next player iteration.
            }
