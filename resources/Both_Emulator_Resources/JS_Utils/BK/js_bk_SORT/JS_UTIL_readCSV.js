...
function main() {
  ...
  let FILENAME_NO_EXT = 'Magneto_Movelist_Original';
  ...
  for (let check = 0; check < allDataArr.length - 1; check++) // length-1 because we're checking the next element
  {
    if ((allDataArr[check + 1][0] === allDataArr[check][0])) // line number is the same
    {
      allDataArr.splice(check + 1, 1); // remove the next line
  ...
  for (let rowIdx = 1; rowIdx < allDataArr.length; ++rowIdx) {
    for (let colIdx = 0; colIdx < headersArray.length; ++colIdx) {
      allArrayStructure[colIdx].push(allDataArr[rowIdx][colIdx]);
    }
  }
