...
import {DIR_PCSX2_CT_FILES} from './JS_UTIL_paths.js';
import {UNUSED_CT_ADDRESSES} from './JS_UTIL_staticData.js';

const CT_FILE_PREFIX = 'pcsx2_entelechy_';
const CT_FILE_SUFFIX = '.CT';
// Get files from directory
var allFileNames = [];
fs.readdirSync(DIR_PCSX2_CT_FILES).forEach((file) =>
{
...
fs.readdirSync(DIR_PCSX2_CT_FILES).forEach((file) =>
{
  ...
  {
    var stats = fs.statSync(`${ DIR_PCSX2_CT_FILES }${ file }`);
...
const READ_CT_FILE = fs.readFileSync(`${ DIR_PCSX2_CT_FILES }${ MAIN_CT_FILE }`.toString(), 'utf8');
var readFlag = false;
...
if (regexForPreviousLuaScript.test(READ_CT_FILE))
{
  ...
  else if (!READ_CT_FILE.includes(textToWrite))
  {
    ...
    fs.writeFileSync(`${ DIR_PCSX2_CT_FILES }${ CTFileNamePlusOne }`, CTFileWithNewLuaScript, 'utf8');
    txtSlot = 1;
...
if (txtSlot == 1)
{
  fs.writeFileSync(`${ DIR_PCSX2_CT_FILES }${ CTFileNamePlusOne }_addresses.txt`, ((`${ cheatEntryArrayFixedSet }`)
    .replace(/,/gmi, '\n') + textToWrite)
    , 'utf8');
  // console.log(txtSlot);
}
...
{
  fs.writeFileSync(`${ DIR_PCSX2_CT_FILES }${ newestCTFile }_addresses.txt`, ((`${ cheatEntryArrayFixedSet }`)
    .replace(/,/gmi, '\n') + textToWrite)
    , 'utf8');
  // console.log(txtSlot);
}