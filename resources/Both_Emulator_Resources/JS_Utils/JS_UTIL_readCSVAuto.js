...
{
  var FILENAME_NO_EXT = DIR_CSVS + answer[i];
  var headersArray = [];
  var allDataArray = [];

  // fs.promises.readFile(FILENAME_NO_EXT, 'utf8').then((data) =>
  // {
  //   data.split('\r\n').map((line, index) =>
  //   {
  //     if (index === 0)
  ...
  if (!(fs.existsSync(`${ DIR_SORTED_JS }${ answer[i].replace(`.csv`, ``) }${ TAIL_TEXT }`))) // if the file doesn't exist
  {
    if (missingEntries.length > 0)
    {
      fs.promises.writeFile(`${ DIR_SORTED_JS }${ answer[i].replace(`.csv`, ``) }${ TAIL_TEXT }`,
    ...
    fs.promises.appendFile(`${ DIR_SORTED_JS }${ answer[i].replace(`.csv`, ``) }${ TAIL_TEXT }`,
      (`${ stringArray.join('\n') }`));

    console.log('Done writing Sorted_JS file for ' + answer[i]);
    // console.log(answer);
  }
  else
  {
