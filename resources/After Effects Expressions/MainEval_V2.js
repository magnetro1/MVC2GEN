...
const clipName = function()
{
	...
	return getNameTrunc + "\r" + "Total_Frames: " + tfCurr + " | Remaining: " + tfRemaining
}
...
{
	Object.values(pointTableP1)[i] == 0 ? P1.push(Object.keys(pointTableP1)[i]) : "eod"
}

//P2
for ( let i = 0 ; i < Object.values(pointTableP2).length ; i++ )
{
	Object.values(pointTableP2)[i] == 0 ? P2.push(Object.keys(pointTableP2)[i]) : "eod"
}
...
const varLookup2 = function(Pz, address)
{
	try
	{
		...
		{
			let dataDir = comp("PathString").layer("MainPathString")("Text")("Source Text");
...
const minFN = function(Pz , address )
{
	...
	for ( i = 0 ; i < addressSearch.length ; i++ )
	{
		...
		$.evalFile(assemble);
		let stringSplit = string.split(",");
		let xMath = ( Math.round ( Math.min.apply ( null , stringSplit.splice ( 0 , stringSplit.length ) ) ) )
		arrMin.push(xMath)
	}
	return arrMin
}
...
const maxFN = function(Pz , address )
{
	...
	for ( i = 0 ; i < addressSearch.length ; i++ )
	{
		...
		$.evalFile(assemble);
		let stringSplit = string.split(",");
		let xMath = ( Math.round ( Math.max.apply ( null , stringSplit.splice ( 0 , stringSplit.length ) ) ) )
		arrMax.push(xMath)
	}
	return arrMax
}
...
function Knockdown_StateFN(Pz) // P1 or P2 
{
...
{	
	...
	{	//Ternary checkers for state active/inactive
		...
		arrFS_Install_1.push( dataAdrObj.FSI_Points[p] == 8 || dataAdrObj.FSI_Points[p] == 9  ? 1 : 0 )
		arrFS_Install_2.push( dataAdrObj.FSI_Points[p] > 9 ? 1 : 0 )
		arrNJ_Air.push( dataAdrObj.Airborne[p] == 2 && dataAdrObj.Knockdown_State[p] == 3 && dataAdrObj.SJ_Counter[p] == 0 ? 1 : 0 )
		arrNJ_Rising.push( dataAdrObj.Airborne[p] == 0 && dataAdrObj.Knockdown_State[p] == 2 && dataAdrObj.SJ_Counter[p] == 0 ? 1 : 0 )
		arrOTG_Extra_Stun.push( dataAdrObj.Knockdown_State[p] == 23 && dataAdrObj.Airborne[p] == 3 ? 1 : 0 )
		arrOTG_Forced_Stun.push( dataAdrObj.Knockdown_State[p] == 32 && dataAdrObj.Airborne[p] == 3 ? 1 : 0 )
		arrOTG_Hit.push( dataAdrObj.Action_Flags[p] == 0 && dataAdrObj.Airborne[p] == 3 && dataAdrObj.Knockdown_State[p] == 32 ? 1 : 0 )
		arrOTG_Roll_Invincible.push( dataAdrObj.Action_Flags[p] == 2 && dataAdrObj.Airborne[p] == 1 && dataAdrObj.Attack_Immune[p] == 1 && dataAdrObj.Knockdown_State[p] == 17 ? 1 : 0 )
		arrOTG_Roll_Stunned.push( dataAdrObj.Action_Flags[p] == 1 && dataAdrObj.Airborne[p] == 3 && dataAdrObj.Knockdown_State[p] == 32 ? 1 : 0 )
		arrProxBlock_Air.push( dataAdrObj.Is_Prox_Block[p] == 6 && dataAdrObj.Knockdown_State[p] == 19 ? 1 : 0 )
		arrProxBlock_Ground.push( dataAdrObj.Is_Prox_Block[p] == 5 && dataAdrObj.Knockdown_State[p] == 18 ? 1 : 0 )
		arrPushblock_Air.push(dataAdrObj.Block_Meter[p] > 0 && dataAdrObj.Animation_Timer_Main[p] < 28 && dataAdrObj.Is_Prox_Block[p] == 6 && dataAdrObj.Action_Flags[p] == 2 ? 1 : 0 ) 
		arrPushblock_Ground.push(dataAdrObj.Block_Meter[p] > 0 && dataAdrObj.Animation_Timer_Main[p] < 28 && dataAdrObj.Is_Prox_Block[p] == 5 && dataAdrObj.Action_Flags[p] == 3 ? 1 : 0 )
		arrRising_Invincibility.push(dataAdrObj.Airborne[p] == 0 && dataAdrObj.Attack_Immune[p] == 1 && dataAdrObj.Knockdown_State[p] == 17 ? 1 : 0 )
		arrSJ_Air.push(dataAdrObj.Airborne[p] == 2 && dataAdrObj.Knockdown_State[p] == 14 && dataAdrObj.SJ_Counter[p] == 1 ? 1 : 0 )
		arrSJ_Counter.push(dataAdrObj.SJ_Counter[p] == 2 ? 1 : 0 )
		arrStun.push(dataAdrObj.Knockdown_State[p] == 32 && dataAdrObj.Is_Prox_Block[p] == 13 ? 1 : 0 )
		arrTech_Hit.push(dataAdrObj.Knockdown_State[p] == 27 ? 1 : 0 )
		arrThrown_Air.push(dataAdrObj.Airborne[p] == 2 && dataAdrObj.Knockdown_State[p] == 31 && dataAdrObj.Is_Prox_Block[p] == 16 ? 1 : 0 )
		arrThrown_Ground.push(dataAdrObj.Airborne[p] == 0 && dataAdrObj.Knockdown_State[p] == 31 && dataAdrObj.Is_Prox_Block[p] == 16 ? 1 : 0 )
	}
