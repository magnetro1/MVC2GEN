...
kDemulBase07111117 = {
	...
	keyboardoffset = {
	Q = 0x37210, W = 0x37211, E = 0x37212, R = 0x37213, T = 0x37214, Y = 0x37215, U = 0x37216, I = 0x37217, O = 0x37218, P = 0x37219,
	A = 0x3721E, S = 0x3721F, D = 0x37220, F = 0x37221, G = 0x37222, H = 0x37223, J = 0x37224, K = 0x37225, L = 0x37226,
	Z = 0x3722C, X = 0x3722D, C = 0x3722E, V = 0x3722F, B = 0x37230, N = 0x37231, M = 0x37232
	}
}
...
if (not hotkeyobjects) then hotkeyobjects = {} end -- used for destroying hotkeys on reload
if (not hitboxobjects) then hitboxobjects = {} end -- contains an address-sprite table key-value pair for hitboxes
if (not objecttable) then objecttable = {} end -- a list of all hitbox object memory addresses
if (not textureobjects) then textureobjects = {} end -- stores textures for updating input sprites
if (not inputspriteobjects) then inputspriteobjects = {} end
if (not inputlog) then inputlog = {} end
...
if (not framedatalog) then framedatalog = {} end -- ADD THIS LINE WITH THE OTHER STATICS
...
function ReadByte(_address)
	...
	if (debugmode) then
		local _debugmessage = string.format("ReadByte; Address: %X, Value: %u, Value(Hex): %X", _address, _value, _value)
		print(_debugmessage)
	end
	return _value
end
...
function WriteByte(_address, _value)
	...
	if (debugmode) then
		local _debugmessage = string.format("WriteByte; Address: %X, Value: %u, Value(Hex): %X", _address, _value, _value)
		print(_debugmessage)
	end
end
...
function ReadShort(_address)
	...
	if (debugmode) then
		local _debugmessage = string.format("ReadShort; Address: %X, Value: %u, Value(Hex): %X", _address, _value, _value)
		print(_debugmessage)
	end
	return _value
end
...
function ReadSignedShort(_address)
	...
	if (bAnd(_value, 0x8000) > 0) then _value = _value - 65536 end
	if (debugmode) then
		local _debugmessage = string.format("ReadSignedShort; Address: %X, Value: %d, Value(Hex): %X", _address, _value, _hexvalue)
		print(_debugmessage)
	end
	return _value
end
...
function ReadInteger(_address)
	...
	if (debugmode) then
		local _debugmessage = string.format("ReadInteger; Address: %X, Value: %u, Value(Hex): %X", _address, _value, _value)
		print(_debugmessage)
	end
	return _value
end
...
function WriteInteger(_address, _value)
	...
	if (debugmode) then
		local _debugmessage = string.format("WriteInteger; Address: %X, Value: %u, Value(Hex): %X", _address, _value, _value)
		print(_debugmessage)
	end
end
...
function GetHitboxIds(_choiceaddr)
	...
	if (debugmode) then
		...
		local _debugmessage = string.format("GetHitboxIds; Choice Address: %X, ID Table: ", _choiceaddr) .. _debugtablelist
		print(_debugmessage)
	end
	return _idtable
end
...
function GetHitboxData(_hitboxid)
	...
	if (debugmode) then
		...
		local _debugmessage = string.format("GetHitboxData; Hitbox ID Address: %X, Data Table: ", _hitboxid) .. _debugtablelist
		print(_debugmessage)
	end
...
function ConvertHitboxData(_datatable)
	...
	if (debugmode) then
		local _debugmessage = string.format("ConvertHitboxData; xmin: %d, xmax: %d, ymin: %d, ymax: %d", _hitboxcoords.xmin, _hitboxcoords.xmax, _hitboxcoords.ymin, _hitboxcoords.ymax)
		print(_debugmessage)
	end
	return _hitboxcoords
end
...
function MoveHitbox(_sprite, _objaddr, _objofftable, _hitboxcoords)
    ...
    if (debugmode) then
        local _debugmessage = string.format("MoveHitbox; Object Address: %X, Is Facing Right: %u", _objaddr, _hitboxflip)
        print(_debugmessage)
    end
    if (_hitboxcoords.xmin == _hitboxcoords.xmax or _hitboxcoords.ymin == _hitboxcoords.ymax or ReadByte(GetRomAddr(romdata.pausemenu)) ~= 0) then
        _sprite.Visible = false
        return
    end
	_coords.x = (readFloat(_objaddr + _objofftable.arenaposx) - readFloat(GetRomAddr(romdata.screenarenax)) + _screenoffsetx) * _screenmultix * internalres
    _coords.y = (readFloat(GetRomAddr(romdata.screenarenay)) - readFloat(_objaddr + _objofftable.arenaposy) + _screenoffsety) * _screenmultiy * internalres
    ...
    if (debugmode) then
        local _debugmessage = string.format("MoveHitbox; Hitbox X: %.3f, Hitbox X min: %.3f, Hitbox X max: %.3f, Hitbox Y: %.3f, Hitbox Y min: %.3f, Hitbox Y max: %.3f", _coords.x, _coords.xmin, _coords.xmax, _coords.y, _coords.ymin, _coords.ymax)
        print(_debugmessage)
    end
end
...
function KeyInputDisable(_bool)
	local _value = 0
	if (_bool) then _value = 1 end
	WriteInteger(getAddress("emuinputaaflag"), _value)
end

function MacroSetIdleHotkey()
	if (ActiveGameWindowCheck()) then MacroSetIdle() end
end
...
function MacroSetPlayHotkey()
	if (ActiveGameWindowCheck()) then MacroSetPlay() end
end
...
function MacroSetLoopHotkey()
	if (ActiveGameWindowCheck()) then MacroSetLoop() end
end
...
function MacroSetRecordHotkey()
	if (ActiveGameWindowCheck()) then MacroSetRecord() end
end
...
function MacroSetState(_kMacroEnum)
	if (macrostate == _kMacroEnum) then return end
...
function MacroRecord()
	...
	if (macroline == 1 and ConsoleForm.MacroFrameToggle.Checked and _fsrate ~= _fscount) then return end
...
function MacroGetCurrentLog(_flagoffseta, _flagoffsetb, _editboxkey)
	...
	if (string.len(_keystring) < 12) then return "" end
	local _inputstring = ""
	...
	if (bAnd(_byteb, 2) > 0) then _inputstring = _inputstring .. string.sub(_keystring, 5, 5) end -- X
	if (bAnd(_byteb, 1) > 0) then _inputstring = _inputstring .. string.sub(_keystring, 6, 6) end -- Y
	if (bAnd(_bytea, 128) > 0) then _inputstring = _inputstring .. string.sub(_keystring, 7, 7) end -- L
	if (bAnd(_bytea, 64) > 0) then _inputstring = _inputstring .. string.sub(_keystring, 8, 8) end -- A
	if (bAnd(_bytea, 32) > 0) then _inputstring = _inputstring .. string.sub(_keystring, 9, 9) end -- B
	if (bAnd(_bytea, 16) > 0) then _inputstring = _inputstring .. string.sub(_keystring, 10, 10) end -- R
	if (bAnd(_byteb, 128) > 0) then _inputstring = _inputstring .. string.sub(_keystring, 11, 11) end -- Start
	if (string.len(_inputstring) < 1) then _inputstring = _inputstring .. string.sub(_keystring, 12, 12) end -- Blank character
	return _inputstring
end
...
function MacroRun()
	...
	if (macroline > ConsoleForm.MacroBoxP1.Items.Count and macroline > ConsoleForm.MacroBoxP2.Items.Count) then
		macroline = 1
		if (macrostate == kMacroEnum.play or (ConsoleForm.MacroBoxP1.Items.Count == 0 and ConsoleForm.MacroBoxP2.Items.Count == 0)) then MacroSetIdle() end
	end
end
...
function MacroInputLetters(_listbox)
	if (macroline > _listbox.Items.Count or macroline < 1) then return end
	...
	for i = 1, string.len(_string) do
		...
		if (_offset) then WriteByte(exedata.inputbase + _offset, _pressbyte) end
	end
end
...
function MacroBoxSelectCurrent(_macrobox)
	...
	if (_currentline > _itemcount) then _currentline = _itemcount end
	_macrobox.ItemIndex = _currentline - 1
end

function MacroBoxAdd()
	local _listbox = ConsoleForm.MacroBoxP1
	if (ConsoleForm.MacroEnableSecondBox.Checked) then _listbox = ConsoleForm.MacroBoxP2 end
...
function MacroBoxDelete()
	...
	if (_selected == _listbox.Items.Count) then _selected = _selected - 1 end
...
function MacroBoxSave()
	...
	if _savedialog.execute() then
		...
		if (_listbox.Items.Count > 0) then _macrofile:write(_listbox.Items[_listbox.Items.Count - 1]) end
		_macrofile:close()
	end
	_savedialog.destroy()
end
...
function MacroStatus()
	local _listbox = ConsoleForm.MacroBoxP1
	if (ConsoleForm.MacroEnableSecondBox.Checked) then _listbox = ConsoleForm.MacroBoxP2 end
	ConsoleForm.MacroStatus.Caption = string.format("Sel line: %d, Total: %d, %s, Macro line: %d", _listbox.ItemIndex + 1, _listbox.Items.Count, _listbox.Name, macroline)
end
...
function InputUpdateSprites()
	if (ConsoleForm.InputLogDisable.Checked) then return end
	d3dobj.beginUpdate()
	InputLogUpdate()
	InputChangeTextures()
	InputResizeSprites()
...
function InputGetCurrentLog(_flagoffseta, _flagoffsetb)
	...
	if (bAnd(_bytea, 96) == 0 and bAnd(_byteb, 3) == 0) then
		...
		if (bAnd(_bytea, 64) > 0) then _4buttons = _4buttons .. "A" end
		if (bAnd(_bytea, 32) > 0) then _4buttons = _4buttons .. "B" end
		if (bAnd(_byteb, 2) > 0) then _4buttons = _4buttons .. "X" end
		if (bAnd(_byteb, 1) > 0) then _4buttons = _4buttons .. "Y" end
	end
	if (bAnd(_bytea, 144) == 0 and bAnd(_byteb, 128) == 0) then
		_3buttons = _3buttons .. "3Blank"
	else
		if (bAnd(_bytea, 128) > 0) then _3buttons = _3buttons .. "L" end
		if (bAnd(_bytea, 16) > 0) then _3buttons = _3buttons .. "R" end
		if (bAnd(_byteb, 128) > 0) then _3buttons = _3buttons .. "S" end
	end
...
function InputRowTest(_rowint, _newrowarr)
	...
	if (InputCompareRows(inputlog[_rowint], _newrowarr)) then
		...
		if (inputlog[_rowint][4] > _framemax) then inputlog[_rowint][4] = _framemax end
	else
...
function InputCompareRows(_oldrow, _newrow)
	if (_oldrow[1] == _newrow[1] and _oldrow[2] == _newrow[2] and _oldrow[3] == _newrow[3]) then return true end
	return false
end
...
function InputChangeTextures()
	for i = 1, #inputlog do
		for j = 1, 3 do
			if (inputspriteobjects[i][j].Texture ~= inputlog[i][j]) then inputspriteobjects[i][j].Texture = inputlog[i][j] end
		end
...
function FrameUpdate()
	...
	InputUpdateSprites()
	FrameStepCheck()
  FramePixelUpdate() -- PLACE LINE INSIDE FRAMEUPDATE FUNCTION
  FrameDataTracking() -- PLACE LINE INSIDE FRAMEUPDATE FUNCTION
end
...
function ActiveGameWindowCheck()
	if (getForegroundProcess() == getOpenedProcessID()) then return true else return false end
end
...
function FramePixelSetup() -- PLACE FUNCTIONS BEFORE SETUP CODE SECTION
    local _palette = 0x0000FF -- red
    local _alpha = 1.0
    local _picture = createPicture()
    local _bmp = _picture.Bitmap
    _bmp.Width = 1
    _bmp.Height = 1
    local _canvas = _bmp.Canvas
    _canvas.setPixel(0, 0, _palette)
    local _texture = d3dobj.createTexture(_picture)
    _picture.destroy()
    local _sprite = d3dobj.createSprite(_texture)
    _sprite.Alphablend = _alpha
    framecounterpixel = _sprite
    framecounterpixel.Width = 2
    framecounterpixel.Height = 2
end
function FramePixelUpdate() -- PLACE FUNCTIONS BEFORE SETUP CODE SECTION
    local _pixelposx = 0
    local _pixelposy = 0
    local _pixelmod = 10 -- amount of different pixel positions
    local _framemod = ReadInteger(GetRomAddr(romdata.totalframes))
    _framemod = _framemod % _pixelmod
    framecounterpixel.X = _pixelposx + _framemod
    framecounterpixel.Y = _pixelposy
end
function FrameDataTracking() -- PLACE FUNCTIONS BEFORE SETUP CODE SECTION
  -- Input the data points you want to record here, data is dumped from bottom to top!
  local address_list = getAddressList() 
  local _datalist = {
  _totalframes = address_list.getMemoryRecordByDescription("Total_Frames").Value,
  _frame_skip_cycle_value = address_list.getMemoryRecordByDescription("Frame_Skip_Cycle_Value").Value,
  _p1_input_dec = address_list.getMemoryRecordByDescription("P1_Input_DEC").Value,
  }
  framedatalog[#framedatalog + 1] = _datalist
end
function FrameDataClear() -- PLACE FUNCTIONS BEFORE SETUP CODE SECTION
  for i = 1, #framedatalog do
      for j = 1, #framedatalog[i] do
          framedatalog[i][j] = nil
      end
      framedatalog[i] = nil
  end
end
function FrameDataDump() -- PLACE FUNCTIONS BEFORE SETUP CODE SECTION
  if (framedatalog == nil) then return end
  local _savedialog = createSaveDialog()
  _savedialog.Title = "Save frame data..."
  _savedialog.DefaultExt = ".txt"
  _savedialog.Filter = "Filename (.txt)|.txt"
  _savedialog.Options = "ofOverwritePrompt, ofCreatePrompt"
  if _savedialog.execute() then
      local _macrofile = io.open(_savedialog.FileName, "w")
      local _dumpline = ""
      print(framedatalog[1])
      print(framedatalog[2])
      for _varname in pairs(framedatalog[1]) do
          _dumpline = _dumpline .. _varname .. ","
      end
      _dumpline = _dumpline:sub(1, -2)
      _macrofile:write(_dumpline, "\n")
      for i = 1, #framedatalog do
          dumpline = ""
          for _framevalue in pairs(framedatalog[i]) do
              _dumpline = _dumpline .. _framevalue .. ","
          end
          _dumpline = _dumpline:sub(1, -2)
          _macrofile:write(_dumpline)
          if (i ~= #framedatalog) then _macrofile:write("\n") end
      end
      _macrofile:close()
  end
  _savedialog.destroy()
...
if (not framecounterpixel) then FramePixelSetup() end -- PLACE LINE IN SETUP CODE SECTION
if (not autoAssemble(kInjectCode)) then print("Counter injection script failed.") end
