...
if stgID[0] == 'S':
    subtractAddress = 0x0CEA0000
elif stgID[0] == 'D':
    subtractAddress = 0x0CE80000
else:
    subtractAddress = 0x00000000
...
while width != 0 and height != 0:
    ...
    pol_asm_file.write(msgStr)
    imageDataType = int.from_bytes(pol_file.read(1), 'little')  # imageDataType
    msgStr = '0x%02X ' % imageDataType
    pol_asm_file.write(msgStr)
    ...
    textureInformation = ('0x%04X' % width, '0x%04X' % height, '0x%02X' % textureType, '0x%08X' % textureLocation)
...
if processZOrder:
    ...
    for count, file in enumerate(fileList):
        ...
        dataSize = os.path.getsize(ogBinOutputDirectory + file)
        pixelAmount = round(dataSize / 2)
        print('    info ::', '{0:d}[0x{0:08X}]'.format(pixelAmount),
              'pixles, sqrt -> {0:d}[0x{0:08X}]'.format(round(np.sqrt(pixelAmount))))
        ...
        cv2.imwrite(convOutputDirectory + out_conv_filename + '_PNG.png', img_RGBA)
        print('')
