...
var allFontsArray = [
  ...
  'SFA3Name1Regular',
  'SFA3Name2Regular',
  // Street Fighter Alpha 3 TTF
  //'Street-Fighter-Alpha',
  //'Street-Fighter-Alpha-3-(Medium)',
  //'Street-Fighter-Alpha-3-(Small)',
...
// Global Variables
var GLOBAL_OUTPUT_FOLDER = 'H:/Git/MVC2GEN/AdobeScripts/Photoshop/' //'H:/';
...
function getDateStamp()
{
  ...
  var dateStamp = year + '_' + month + '_' + day + '_' + hours + '_' + minutes + '_' + seconds;
  return dateStamp;
}
/**
...
function createFontReferencePNGs()
{
  ...
  for (var i = 0; i < allFontsArray.length; i++)
  {
    var iterativeEntry = allFontsArray[i]
    ...
    try
    {
      ...
      // Continue, don't overwrite if file exists
...
/**
 * 
 * @param {string} funcFont get font name from allFontsArray or other font objects
...
/**
 *
 * @param {string} funcFont get font name from allFontsArray
 * @param {number} funcSize set font size of point text
 * @param {string} funcExtension set PNG || PSD
 */
function writePointText(funcFont, funcSize, funcExtension)
{
  var newDocument = app.documents.add(4000, 4000, 72, iterativeEntry, NewDocumentMode.RGB, DocumentFill.TRANSPARENT);
  var layers = newDocument.artLayers;
  var newLayer = layers.add();
...
function writeParagraphText(funcFont, funcSize, funcExtension, promptOrNot)
{
  ...
  paragraphTextItem.kind = TextType.PARAGRAPHTEXT; // TextType.PARAGRAPHTEXT || TextType.POINTTEXT
...
function saveLogic(funcExtension)
{
  if (funcExtension == 'PNG' || funcExtension == 'png')
  {
    var saveFilePNG = new File(new File(GLOBAL_OUTPUT_FOLDER + '/' + getDateStamp().toString() + '.png'));
    SavePNG(saveFilePNG);
  }
  else if (funcExtension == 'PSD' || funcExtension == 'psd')
  {
    var saveFilePSD = new File(new File(GLOBAL_OUTPUT_FOLDER + '/' + getDateStamp().toString() + '.psd'));
    SavePSD(saveFilePSD);
  }
}
...
// createAllCharacterTitlesPNGs(fontsMainObj.MVC2);
// for (var tempFont in fontsMainObj)
// {
//   createAllCharacterTitlesPNGs(fontsMainObj[tempFont]);
// }

// writePointText(funcFont, funcSize, funcExtension)
...
// writeParagraphText(fontsSubOneObj.MVC2, 72, 'png', false);