...
import(ORG_JS_FILE)
  .then((pMem) => // Imports SortedJS file as Object with key-value pairs
  {
    const CLIP_LENGTH = pMem.A_2D_Game_Timer.split(',').length;
    for (let adr in MIN_MAX_ADRS)
    {
      const KEY = MIN_MAX_ADRS[adr];
      const VALUE = pMem[MIN_MAX_ADRS[adr]].split(','); // Fetch the value by finding the key using its string name, then make into an array by splitting it.
    ...
    let tempRegExVar;
    while (tempRegExVar = allVariablesREGEX.exec(oldData))
    {
    ...
    function getLabelsfromJS()
    {
      ...
      let tempRegExVar;
      let newFile = fs.readFileSync(ORG_JS_FILE, 'utf8');
